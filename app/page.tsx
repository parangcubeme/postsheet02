...
type SmartSettings = {
  feeRate: number; marginRate: number; extraCost: number; roundUnit: number;
  categoryCode: string; courierCode: string; asPhone: string; multipleOrigins: "N" | "Y";
};
type EsmSettings = {
  marginRate: number; roundUnit: number; auctionId: string; gmarketId: string; courierName: CourierName;
};
type EsmPreviewProduct = Product & { finalPrice: number };
...
function toProducts(rows: Row[]): Product[] {
  return rows.map((raw) => ({ raw, productName: normalize(pick(raw, aliases.productName)), sellerCode: normalize(pick(raw, aliases.sellerCode)), basePrice: numberValue(pick(raw, aliases.basePrice)), stock: numberValue(pick(raw, aliases.stock)) || 99999, optionName: normalize(pick(raw, aliases.optionName)), optionValue: normalize(pick(raw, aliases.optionValue)), mainImage: normalize(pick(raw, aliases.mainImage)), additionalImage: getAdditionalImage(raw), detailHtml: normalize(pick(raw, aliases.detailHtml)), shippingFee: numberValue(pick(raw, aliases.shippingFee)), vatType: normalize(pick(raw, aliases.vatType)), originCode: normalize(pick(raw, aliases.originCode)), originDirect: normalize(pick(raw, aliases.originDirect)) })).filter((product) => (product.productName || product.sellerCode) && product.basePrice > 0);
}
function roundedPrice(base: number, margin: number, unit: number, extra = 0) { if (base <= 0) return 0; const safeUnit = unit > 0 ? unit : 1; return Math.ceil(((base + extra) * (1 + margin / 100)) / safeUnit) * safeUnit; }
...
function esmRow(product: Product, settings: EsmSettings) {
  const raw = product.raw; const price = roundedPrice(product.basePrice, settings.marginRate, settings.roundUnit); const row: unknown[] = Array(esmHeaders.length).fill("");
  ...
  set("카테고리 템플릿 코드", copyCode(raw, ["카테고리 템플릿 코드", "카테고리템플릿코드"])); set("카테고리 코드", copyCode(raw, ["ESM 카테고리코드", "카테고리 코드", "카테고리코드"])); set("A 노출코드", copyCode(raw, ["A 노출코드", "옥션 노출코드"])); set("G 노출코드", copyCode(raw, ["G 노출코드", "G마켓 노출코드"]));
  set("판매기간", "무제한"); set("A 판매가", price); set("G 판매가", price); set("A 재고", product.stock); set("G 재고", product.stock);
  set("옵션\n타입", esmOptionType(product)); set("옵션명", product.optionName); set("옵션\n입력값", product.optionValue); set("기본이미지", product.mainImage); set("추가이미지", product.additionalImage); set("상품상세설명", product.detailHtml);
...
function makeEsmWorkbook(rows: Product[], settings: EsmSettings) { const sheet = XLSX.utils.aoa_to_sheet([...esmTopRows, ...rows.map((product) => esmRow(product, settings))]); sheet["!cols"] = Array(64).fill({ wch: 16 }); sheet["!rows"] = [{ hpt: 24 }, { hpt: 24 }, { hpt: 24 }, { hpt: 50 }, { hpt: 24 }, { hpt: 120 }, { hpt: 32 }]; const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, sheet, "NEW 일반상품"); return workbook; }
...
export default function Home() {
  ...
  const [esmMargin, setEsmMargin] = useState(30); const [esmRound, setEsmRound] = useState(100); const [auctionId, setAuctionId] = useState(""); const [gmarketId, setGmarketId] = useState(""); const [esmCourier, setEsmCourier] = useState<CourierName>("CJ대한통운"); const [esmApplied, setEsmApplied] = useState<EsmSettings | null>(null);
  const esmPreview = useMemo<EsmPreviewProduct[]>(() => products.slice(0, 30).map((product) => ({ ...product, finalPrice: roundedPrice(product.basePrice, esmMargin, esmRound) })), [products, esmMargin, esmRound]);

  async function readFile(file?: File) { if (!file) return; setFileName(file.name); setSmartApplied(null); setEsmApplied(null); try { const workbook = XLSX.read(await file.arrayBuffer(), { type: "array" }); const rows = parseSheet(workbook.Sheets[workbook.SheetNames[0]]); const parsedProducts = toProducts(rows); setSourceRows(rows); setStatus(`${rows.length}행을 읽었고 판매가가 확인된 상품은 ${parsedProducts.length}개입니다.`); } catch { setSourceRows([]); setStatus("파일을 읽지 못했습니다."); } }
  ...
  function applyEsm() { if (!products.length) return; if (!auctionId.trim() && !gmarketId.trim()) { setStatus("옥션 또는 G마켓 판매자 ID를 하나 이상 입력해 주세요."); return; } const settings: EsmSettings = { marginRate: esmMargin, roundUnit: esmRound, auctionId: auctionId.trim(), gmarketId: gmarketId.trim(), courierName: esmCourier }; setEsmApplied(settings); const sample = products[0]; setStatus(`ESM 적용 완료 · 예시 ${sample.basePrice.toLocaleString()}원 → ${roundedPrice(sample.basePrice, esmMargin, esmRound).toLocaleString()}원 · ${Math.ceil(products.length / 500)}개 파일`); }
  async function downloadEsm() { if (!esmApplied) return; const chunks: Product[][] = []; for (let index = 0; index < products.length; index += 500) chunks.push(products.slice(index, index + 500)); const date = new Date().toISOString().slice(0, 10); if (chunks.length === 1) { XLSX.writeFile(makeEsmWorkbook(chunks[0], esmApplied), `ESM_${chunks[0].length}개_${date}.xlsx`); return; } const zip = new JSZip(); chunks.forEach((chunk, index) => { const output = XLSX.write(makeEsmWorkbook(chunk, esmApplied), { bookType: "xlsx", type: "array" }); zip.file(`ESM_${String(index + 1).padStart(2, "0")}_${chunk.length}개.xlsx`, output); }); downloadBlob(await zip.generateAsync({ type: "blob" }), `ESM_${products.length}개_분할파일_${date}.zip`); }
